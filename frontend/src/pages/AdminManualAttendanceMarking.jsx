...
const AdminManualAttendance = () => {
  ...
  const handleExcelUpload = async (event) => {
    ...
    try {
      ...
      if (headerRowIndex === -1) {
        showToast("Could not find Intern ID / Status columns", "error");
        return;
      }
      ...
      for (let i = headerRowIndex + 1; i < rows.length; i++) {
        ...
        if (internId.toLowerCase().includes("attendance summary")) {
          break;
        }

        if (internId && status.toLowerCase() === "present") {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-[1fr_1fr_400px] gap-5 sm:gap-6">
              ...
              <div className="space-y-5 lg:col-span-2 xl:col-span-2">
                ...
                <motion.button
                  ...
                  style={{
                    background:
                      canSubmit && !marking ? submitGradient : "#d1d5db",
                  }}
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
