...
const leftSection = {
  ...
  outerRing1: [
    ...
    { number: 35, angle: 176, radius: 225 },
    { number: 36, angle: 160, radius: 195 },
    { number: 0, angle: 140, radius: 180 },
    { number: 37, angle: 120, radius: 174 },
    { number: 38, angle: 100, radius: 170 },
    { number: 39, angle: 80, radius: 170 },
    { number: 40, angle: 60, radius: 174 },
    { number: 41, angle: 40, radius: 180 },
    { number: 42, angle: 20, radius: 195 },
    { number: 43, angle: 4, radius: 225 },
    { number: 44, angle: -10, radius: 240 },
  ],
  outerRing2: [
  ...
  outerRing3: [
    { number: 53, angle: 75, radius: 300 },
    { number: 54, angle: 62, radius: 310 },
    { number: 55, angle: 48, radius: 320 },
  ],
};
...
const rightSection = {
  ...
  outerRing1: [
    { number: 91, angle: 160, radius: 280 },
    { number: 90, angle: 147, radius: 275 },
    { number: 89, angle: 134, radius: 270 },
    { number: 87, angle: 120, radius: 260 },
    { number: 86, angle: 106, radius: 250 },
    { number: 85, angle: 92, radius: 245 },
    { number: 84, angle: 78, radius: 250 },
    { number: 83, angle: 64, radius: 260 },
    { number: 82, angle: 50, radius: 270 },
    { number: 81, angle: 36, radius: 275 },
    { number: 80, angle: 22, radius: 280 },
  ],
  outerRing2: [
    { number: 70, angle: 177, radius: 240 },
    ...
    { number: 79, angle: 3, radius: 240 },
  ],
  outerRing3: [
    { number: 92, angle: 132, radius: 340 },
    { number: 93, angle: 120, radius: 330 },
    { number: 94, angle: 50, radius: 340 },
    { number: 95, angle: 38, radius: 345 },
    { number: 96, angle: 26, radius: 350 },
  ],
};
...
export const useSeatManagement = () => {
  ...
  const fetchBookingsByDate = useCallback(async (dateString) => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (Array.isArray(result)) {
        result.forEach((booking) => {
          transformedBookings[booking.seatNumber] = {
  ...
  const loadBookingsForDate = useCallback(
    async (dateString) => {
      const bookings = await fetchBookingsByDate(dateString);
  ...
  const handleSeatClick = useCallback(
    (seatNumber) => {
      ...
      if (dailyBookings[seatNumber]) {
        alert(
          `This seat is already booked for ${selectedDate} by Intern ID: ${dailyBookings[seatNumber].internId}`
        );
        return;
      }
  ...
  }, [currentSeat, selectedDate, createBooking, handleModalClose]);
  ...
  const handleCancelBooking = useCallback(
    async (seatNumber) => {
      ...
      try {
        ...
        const updatedBookings = { ...dailyBookings };
        delete updatedBookings[seatNumber];
        setDailyBookings(updatedBookings);
