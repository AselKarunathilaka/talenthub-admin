...
path = 'g:/github/TalentHub/frontend/src/pages/InternSeatReservation.jsx'
with open(path, 'r', encoding='utf-8') as f:
    code = f.read()

search = """                <div className="flex-1 min-w-[160px] bg-slate-50 rounded-xl p-2 flex items-center gap-2 border border-slate-100">
                  <div className="bg-white p-1.5 rounded-lg shadow-sm border border-slate-100"><Calendar className="text-[#00b4eb] h-4 w-4" /></div>
        {showModal && <BookingModal"""

replace = """                <div className="flex-1 min-w-[160px] bg-slate-50 rounded-xl p-2 flex items-center gap-2 border border-slate-100">
                  <div className="bg-white p-1.5 rounded-lg shadow-sm border border-slate-100"><Calendar className="text-[#00b4eb] h-4 w-4" /></div>
...
if search in code:
    ...
    print("Could not find search string in file. Let's do a more robust search.")
    
    # Try finding the indices directly
    import re
    match = re.search(r'bg-white p-1\.5 rounded-lg shadow-sm border border-slate-100.*?\{showModal && <BookingModal', code, re.DOTALL)
    if match:
        start = match.start()
        end = match.end()
        # Create replacement string using the matched first part and the expected second part
        new_str = match.group(0).replace('{showModal && <BookingModal', replace.split('</div>\n        {showModal && <BookingModal')[0] + '</div>\n        {showModal && <BookingModal')
        code = code[:start] + new_str + code[end:]
        with open(path, 'w', encoding='utf-8') as f:
            f.write(code)
        print("Replaced successfully using regex")
    else:
        print("Could not find using regex either")
