...
// 1. Replace the first react-icons import accurately
const iconsImportTarget = `import {
  FaChair,
  FaUsers,
  FaCheckCircle,
  FaCalendarAlt,
...
content = content.replace(iconsImportTarget, newIconsImport);
...
// 4. Remove activeSection unused state
content = content.replace('const [activeSection, setActiveSection] = useState("A");', '');
content = content.replace("const [activeSection, setActiveSection] = useState('A');", '');

// 5. Remove unused showLockManager state
content = content.replace('const [showLockManager, setShowLockManager] = useState(true);', '');

// 6. Split at the correct point
...
const beforeReturn = content.split(splitPoint)[0];
...
console.log('Update perfect complete');
