...
const regex = /<div className="bg-white p-1\.5 rounded-lg shadow-sm border border-slate-100"><Calendar className="text-\[#00b4eb\] h-4 w-4" \/><\/div>[\s\S]*?\{showModal && <BookingModal/;
const match = code.match(regex);
if (match) {
    const newCode = code.replace(regex, replacement.split('<div className="bg-white p-1.5 rounded-lg shadow-sm border border-slate-100"><Calendar className="text-[#00b4eb] h-4 w-4" /></div>')[1].trim());
    
    // We actually just want to replace everything from '<div className="flex-1 min-w-[160px] bg-slate-50...' to '{showModal...'
    const regex2 = /<div className="flex-1 min-w-\[160px\] bg-slate-50 rounded-xl p-2 flex items-center gap-2 border border-slate-100">[\s\S]*?\{showModal && <BookingModal/;
    const newCode2 = code.replace(regex2, replacement);
    
    fs.writeFileSync(path, newCode2);
