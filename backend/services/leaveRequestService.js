...
const getLeaveTraineeId = (leave) => leave?.intern?.Trainee_ID || leave?.internTraineeId || null;
...
class LeaveRequestService {
  async createLeaveRequest(internId, leaveRequestData) {
    try {
      ...
      console.log(`Creating leave request for intern: ${internId}, Trainee_ID: ${intern.Trainee_ID}`);
      ...
      console.log(`Leave request created by intern ${internId}, stored Trainee_ID: ${leaveRequest.internTraineeId}`);
      return leaveRequest;
    } catch (error) {
  ...
  async generateApprovedLeavesPDF({ startDate, endDate }) {
    const leaveRequests = await this.getApprovedLeavesByDate(startDate, endDate);
    ...
    doc.fontSize(18).text('Approved Short Leave Requests Report', { align: 'center' });
    doc.moveDown(0.5);
    const startLabel = startDate ? formatDateLabel(startDate) : 'Beginning';
    const endLabel = endDate ? formatDateLabel(endDate) : 'Present';
    doc.fontSize(12).text(`Reporting window: ${startLabel} – ${endLabel}`, { align: 'center' });
    doc.fontSize(12).text(`Generated: ${formatDateLabel(new Date())}`, { align: 'center' });
    doc.moveDown(1.5);

    if (leaveRequests.length === 0) {
      doc.fontSize(12).text('No approved short leave requests were recorded for the selected period.', {
        align: 'left'
      });
      doc.end();
    ...
    sortedDates.forEach((dateKey, index) => {
      ...
      doc.fontSize(14).fillColor('#0b5394').text(formatDateLabel(dateKey, { includeWeekday: true }), {
        underline: true
      });
      doc.fontSize(12).fillColor('black').text(`Total approved: ${entries.length}`);
      doc.moveDown(0.25);

      entries.forEach((leave) => {
        const internFullName = leave.intern?.Trainee_Name || leave.internName || 'Unknown Intern';
        const rawTraineeId = getLeaveTraineeId(leave);
        const traineeId = (rawTraineeId && rawTraineeId !== 'N/A') ? rawTraineeId : 'N/A';
        ...
        doc.font('Helvetica').fontSize(11).text(`Time: ${time} | Purpose: ${purpose}`, {
          indent: 10
        });
        doc.font('Helvetica').fontSize(11).text(`Reason: ${reason}`, { indent: 10 });
        doc.moveDown(0.75);
      });
  ...
  async notifyInternStatusUpdate(leaveRequest) {
    try {
      ...
      const subject = `Short Leave Request ${statusText} - TalentHub`;
      
      const html = `
        <h2>Short Leave Request ${statusText}</h2>
        <p>Your short leave request has been ${statusText.toLowerCase()}.</p>
