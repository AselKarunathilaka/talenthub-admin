...
class LeaveRequestService {
  ...
  async generateApprovedLeavesPDF({ startDate, endDate }) {
    ...
    doc.fontSize(18).text('Approved Leave Requests Report', { align: 'center' });
    doc.moveDown(0.5);
    ...
    if (leaveRequests.length === 0) {
      doc.fontSize(12).text('No approved leave requests were recorded for the selected period.', {
        align: 'left'
      });
      doc.end();
    ...
    sortedDates.forEach((dateKey, index) => {
      ...
      entries.forEach((leave) => {
        const internFullName = leave.intern && (leave.intern.firstName || leave.intern.lastName)
          ? `${leave.intern.firstName || ''} ${leave.intern.lastName || ''}`.trim()
          : leave.internName || 'Unknown Intern';
        const nic = leave.intern?.nic || leave.internNIC || 'N/A';
        ...
        doc.font('Helvetica').fontSize(11).text(`NIC: ${nic} | Time: ${time} | Purpose: ${purpose}`, {
          indent: 10
        });
