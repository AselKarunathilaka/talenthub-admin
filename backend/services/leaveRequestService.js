...
class LeaveRequestService {
  async calculateWorkingDays(startDate, endDate) {
    ...
    const holidays = await Holiday.find({ date: { $in: dates } }).lean();
    ...
    for (const d of dates) {
      ...
      if (day !== 0 && day !== 6 && !holidayDates.has(d)) {
        workingDays++;
      }
    }
    return workingDays;
  ...
  async createLeaveRequest(internId, leaveRequestData) {
    try {
      ...
      if (requestType === "study_leave") {
        ...
        // Quota Check
        const requestedWorkingDays = await this.calculateWorkingDays(leaveDate, studyEndDate);
        
        // Find existing leaves this year
        const currentYear = leaveDate.getFullYear();
        const existingLeaves = await leaveRequestRepository.findStudyLeavesForYear(internId, currentYear);
        
        let usedWorkingDays = 0;
        for (const leave of existingLeaves) {
          usedWorkingDays += await this.calculateWorkingDays(leave.leaveDate, leave.studyEndDate);
        }

        const studyLeaveQuota = intern.studyLeaveQuota !== undefined ? intern.studyLeaveQuota : 60;
        
        if (usedWorkingDays + requestedWorkingDays > studyLeaveQuota) {
          throw new Error(
            `Extended leave quota exceeded. You have ${studyLeaveQuota - usedWorkingDays} days remaining for ${currentYear}, but requested ${requestedWorkingDays} days.`,
          );
        }
      } else {
  ...
  async notifyAdminNewRequest(leaveRequest) {
    try {
      ...
      const subject = `New ${requestLabel} Request - ${leaveRequest.internName}`;
      const html = `
        <h2>New ${requestLabel} Permission Request</h2>
        <p>A new ${requestLabel.toLowerCase()} request has been submitted and requires your review.</p>
        <h3>Details:</h3>
        <ul>
  ...
  async syncApprovedStudyLeaveToLogbook(leaveRequest) {
    ...
    const workingDates = dates.filter((d) => {
      const dateObj = new Date(d);
      const day = dateObj.getDay();
      return day !== 0 && day !== 6 && !holidayDates.has(d);
    });
