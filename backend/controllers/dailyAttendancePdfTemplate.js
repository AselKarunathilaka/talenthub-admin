...
// First-page header height (logo bar + teal stripe + title + 3 meta lines + divider)
const FIRST_PAGE_HEADER_H = 172;
// Continuation-page column header row height
...
// ── Footer: drawn INSIDE the page boundary to prevent blank overflow pages ────
function drawFooter(doc, pageNum, totalPages) {
  ...
  doc
    ...
    .fillColor(TEXT_MID)
    .text(
      `TalentHub Intern Management System  •  Daily Attendance  •  Page ${pageNum} of ${totalPages}`,
      PAGE_MARGIN,
      footerY,
...
// ── First-page header (logo + title + meta) ───────────────────────────────────
function drawFirstPageHeader(doc, date, totalCount, sortedBy) {
  ...
  doc
    ...
    .fontSize(18)
    .fillColor(TEXT_DARK)
    .text("Intern Attendance Report", x, y);
  y += 28;
...
// ── Main export ───────────────────────────────────────────────────────────────
async function generateDailyAttendancePdf({ date, interns, sortedBy }) {
  return new Promise((resolve, reject) => {
    try {
      // No bufferPages — single forward pass with pre-calculated total pages
      ...
      // ── Step 2: render in a single forward pass ──
      const usableFirst =
        PAGE_HEIGHT -
        PAGE_MARGIN -
        FIRST_PAGE_HEADER_H -
      ...
      // Page 1 — logo + full header
      doc.addPage();
      let y = drawFirstPageHeader(doc, date, interns.length, sortedBy);
      y += drawRow(
        doc,
        COLS.map((c) => c.label),
        PAGE_MARGIN,
      ...
      while (rowIndex < rows.length) {
        ...
        if (remaining - h < 0) {
          // Stamp footer, start new page — NO logo/header, just column headers
