...
const AdminSeatManagement = () => {
  ...
  const handleSearchBookingHistory = async () => {
    if (!searchQuery.trim()) {
      seatNotificationUtils.showInfo(
        "Please enter a trainee ID or name to search",
      );
      return;
    }
    ...
    try {
      ...
      if (response.bookings && response.bookings.length > 0) {
        seatNotificationUtils.showSuccess(
          `Found ${response.bookings.length} booking${response.bookings.length !== 1 ? "s" : ""} for ${response.internInfo?.internName || searchQuery}`,
        );
      } else {
        seatNotificationUtils.showInfo(
      ...
      seatNotificationUtils.showError(
        err.message || "Failed to fetch booking history",
      );
      setSearchResults(null);
    } finally {
                  ...
                  >
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="flex-1">
                        ...
                        {searchResults.internInfo && (
                          <div className="text-sm text-blue-800 space-y-1">
                            <p>
                              <span className="font-medium">Name:</span>{" "}
                              {searchResults.internInfo.internName}
                            </p>
                            <p>
                              <span className="font-medium">Trainee ID:</span>{" "}
                              {searchResults.internInfo.traineeId}
                            </p>
                            <p>
                              <span className="font-medium">Email:</span>{" "}
                              {searchResults.internInfo.email}
                            </p>
                            <p>
